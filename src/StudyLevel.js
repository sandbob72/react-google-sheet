...
class EntMethodDesc extends React.Component {
  constructor() {
    super()
    this.state = {
              ...
              {students.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((item,index) => (
                <TableRow key={index}>
                  <TableCell component="th" scope="row">
                    {item.STUDY_LEVEL_NAME_ENG}
                  </TableCell>
                  <TableCell align="right">{item.Number_study_level}</TableCell>
                </TableRow>
              ))}
              {emptyRows > 0 && (
...
EntMethodDesc.propTypes = {
  classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(EntMethodDesc);